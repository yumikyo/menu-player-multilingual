...
# HTMLプレイヤー生成（修正版：f-stringを使わずreplaceを使用）
...
function ren(){
    ...
    pl.forEach((t,i)=>{
        ...
        m.setAttribute("aria-label", (i+1)+". "+t.title);
...
def render_preview_player(tracks, lang_settings):
    ...
    function rn(){ls.innerHTML="";pl.forEach((t,i)=>{
        ...
        d.onclick=()=>{ld(i);au.play();pb.innerText="⏸";pb.setAttribute("aria-label","__UI_PAUSE__");};
...
st.caption("Multilingual Accessibility Menu Creator")
...
if input_method == "📂 アルバムから":
    ...
    if st.session_state.retake_index is not None:
        ...
        c1, c2 = st.columns(2)
        with c1:
            if camera_file and st.button("✅ これで決定", type="primary", key="retake_confirm"):
...
if st.button("🎙️ 作成開始 / Start", type="primary", use_container_width=True, disabled=disable_create):
    ...
    with st.spinner(f'Analyzing & Translating to {selected_lang}...'):
        try:
            ...
            menu_data.insert(0, {"title": "Intro / Index", "text": intro_t})
